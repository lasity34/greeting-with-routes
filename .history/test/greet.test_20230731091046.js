...
describe('Greeting function', function() {
    ...
    it('This should return a greeting after the name is entered', async function(done) { // Use async here
        try {
            await greeting.setLanguage("English"); // Use await here
            const message = await greeting.greetMessage('bjorn'); // Use await here
            console.log('GreetMessage result:', message);
            assert.equal('Hello, bjorn', message);
            done(); // Successfully done
        } catch (err) {
            done(err); // Pass error to done if there's an issue
        }
    });
